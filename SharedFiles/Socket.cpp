#include "stdafx.h"//TODO: Turn on SDL, need to fix gethostbyname and inet_ntoa
...
{
	// Allocate buffers
	m_readBuffer.Allocate(recvbuffersize);
	m_writeBuffer.Allocate(sendbuffersize);
...
void Socket::WriteCallback()
{
	...
	if (m_writeBuffer.GetContiguousBytes())
	{
		DWORD w_length = 0;
		DWORD flags = 0;
		...
		WSABUF buf;
		buf.len = (ULONG)m_writeBuffer.GetContiguousBytes();
