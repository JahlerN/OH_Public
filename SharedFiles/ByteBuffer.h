...
public:
	...
	bool m_doubleByte;//TODO: I don't like this, but hero strings are usually 1 byte length afaik alteast.

	ByteBuffer() : _rpos(0), _wpos(0), m_doubleByte(false) { _storage.reserve(DEFAULT_SIZE); }
	ByteBuffer(size_t res) : _rpos(0), _wpos(0), m_doubleByte(false) { _storage.reserve(res <= 0 ? DEFAULT_SIZE : res); }
